...
window.addEventListener('DOMContentLoaded', function (event) {
  var downloadButton = document.getElementById('download-ics');
  var form = document.getElementById('event-data');
  var sendMail = document.getElementById('send-email');
  ...
  function sendRequest(sendEmail) {
    ...
    var title = form.querySelector('input[name="summary"]').value;
    var sanitizedTitle = title.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');
...
window.addEventListener('DOMContentLoaded', function (event) {
  ...
  var timezone = document.querySelector('.current-timezone');
  var offsetElem = form.querySelector('[name="utc_offset_minutes"]');
  console.log(Intl.DateTimeFormat());
  autocomplete.addListener('place_changed', function () {
    var place = autocomplete.getPlace();
    var offsetInMinutes = place.utc_offset_minutes;

    if (offsetInMinutes !== undefined && offsetInMinutes !== null) {
      var offset = offsetInMinutes / 60;
      timezone.innerHTML = 'Picked location (UTC' + (offset <= 0 ? "" : "+") + offset + ') (<a href="#" data-action="reset-timezone">reset</a>)'; // Set offset
      ...
      locationInput.dispatchEvent(_event);
    }
  });
});
"use strict";
...
window.addEventListener('DOMContentLoaded', function (event) {
  ...
  function syncData(e) {
    ...
    if (source.dataset.source !== '') {
      var target = invite.querySelector('[data-target="' + source.dataset.source + '"]'); // Target exists.

      if (target !== null) {
        // Source is not empty.
        if (source.value !== '') {
          var newValue = source.value; // Combine start/end

          if (source.dataset.source === 'when') {
            var date_start = form.querySelector('[name="date_start"]').nextSibling;
            var date_end = form.querySelector('[name="date_end"]').nextSibling;
...
window.addEventListener('DOMContentLoaded', function (event) {
  var form = document.getElementById('event-data');
  var timezoneElem = document.querySelector('.current-timezone');
  var timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  var offsetElem = form.querySelector('[name="utc_offset_minutes"]');
  var offset_in_minutes = new Date().getTimezoneOffset(); // Offset in hours

  var offset = offset_in_minutes / 60;
  var opposite_offset = offset * -1;
  var output = timezone + ' (UTC' + (opposite_offset <= 0 ? "" : "+") + opposite_offset + ')'; // Set timezone

  timezoneElem.innerHTML = output; // Set offset element

  offsetElem.value = offset_in_minutes * -1; // Reset timezone to browser default.
